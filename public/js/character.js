class Snake {
  ...
  makeTurn(euler) {
    ...
    this.mesh.rotation.setFromQuaternion(
      this.mesh.quaternion.multiply(this.rotateQuaternion.setFromEuler(euler))
    );
  }

  pitchUp() {
    this.makeTurn(
      this.rotateEuler.fromArray(
        rotationLookup.up[Number(Math.round(this.direction[1]) !== 1)]
      )
    );
  ...
  pitchDown() {
    this.makeTurn(
      this.rotateEuler.fromArray(
        rotationLookup.down[Number(Math.round(this.direction[1]) !== -1)]
      )
    );
  ...
  left() {
    this.makeTurn(
      this.rotateEuler.fromArray(
        rotationLookup.left[Math.round(this.direction[1])]
      )
    );
  ...
  right() {
    this.makeTurn(
      this.rotateEuler.fromArray(
        rotationLookup.right[Math.round(this.direction[1])]
      )
    );
  ...
  move() {
    this.direction.map((dirNormal, i) => {
      if (dirNormal) {
        ...
        this.mesh.position[this.dirs[i]] = THREE.Math.clamp(
          newPosition,
          -offset,
          offset
        );
        ...
        if (
          this.autoRedirect &&
          ((i !== 1 && newPosition >= level.size.horizCenter) ||
            newPosition <= -level.horizCenter ||
            (i === 1 && newPosition >= level.size.vertCenter) ||
            newPosition <= -level.vertCenter)
        ) {
          [this.right, this.left, this.pitchDown, this.pitchUp][
            Math.floor(Math.random() * (level.size.y === 1 ? 2 : 4))
          ].bind(this)();
        }
      }
    });
  ...
  updatePositionInfo() {
    ...
    this.position = position.map((p) => Math.floor(p + 0.1));
...
class Tail {
  constructor() {
    this.vertebra = new THREE.Mesh(
      new THREE.BoxBufferGeometry(0.95, 0.95, 0.95),
      tjs_materials.snake_body
    );
    let scope = this;
      ...
      setTimeout(
        () =>
          vertebra.position.set(
            i - Math.floor((this.vertebrae.length || 1) / 2),
            0,
            -level.size.horizCenter - 1
          ),
        100
      );
    });
  ...
  add() {
    ...
    this.vertebra.name = `tail${String(this.vertebrae.length).padStart(
      2,
      "0"
    )}`;
  ...
  move() {
    if (
      ...
      this.vertebrae.length
    ) {
      this.vertebrae.map((tail, i) => {
        if (this.trailInterpolated[(i + 1) * MOVE_TICKER_COMPARE]) {
          let trailValue = this.trailInterpolated[
            (i + 1) * MOVE_TICKER_COMPARE
          ];
  ...
  update() {
    if (
      ...
      this.trailInterpolated.push([
        player.mesh.position.toArray(),
        player.mesh.quaternion.clone(),
      ]);
      this.trailInterpolated.length >
        (this.vertebrae.length + 1) * MOVE_TICKER_COMPARE &&
        this.trailInterpolated.shift();
      this.trailInterpolated.length =
        (this.vertebrae.length + 1) * MOVE_TICKER_COMPARE;
    }
  }
...
class Compass {
  ...
  buildCurveFromCoordinates(coordinates) {
    const geometry = new THREE.BufferGeometry().setFromPoints(
      coordinates.map((pos) => new THREE.Vector3(...pos))
    );
  ...
  orient() {
    ...
    this.arrowL.position.y = -0.2;
    this.arrowR.position.y = -0.2;
  ...
  move() {
    ...
    let planeVector = new THREE.Vector3(0, 0, 1).applyQuaternion(
      player.mesh.quaternion
    );
    let cameraVector = new THREE.Vector3(0, 0, -1).applyQuaternion(
      tjs_camera.quaternion
    );
    this.facingCamera = planeVector.angleTo(cameraVector) > (3 * Math.PI) / 4;

    if (CLOCK.elapsedTime > 15 && tjs_materials.arrow.opacity)
      tjs_materials.arrow.opacity -= 0.01;
