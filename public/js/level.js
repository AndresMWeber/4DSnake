class Level {
  ...
  buildLevel() {
    ...
    this.floor = new THREE.Mesh(
      new THREE.PlaneBufferGeometry(this.size.x, this.size.z, 0),
      tjs_materials.flat_blue
    );
    this.floor.name = "FloorPlane";
    this.floor.rotateX(-HALF_PI);
    this.floor.translateZ(-this.size.vertCenter - 0.5);

    this.floorZ = new THREE.Mesh(
      new THREE.PlaneBufferGeometry(this.size.x, this.size.y, 0),
      tjs_materials.flat_blue
    );
    this.floorZ.name = "FloorPlaneZ";
    this.floorZ.translateZ(-this.size.horizCenter + -0.5);

    this.floorZP = new THREE.Mesh(
      new THREE.PlaneBufferGeometry(this.size.x, this.size.y, 0),
      tjs_materials.flat_blue
    );
    this.floorZP.name = "FloorPlaneZP";
    this.floorZP.translateZ(this.size.horizCenter + 0.5);
    this.floorZP.rotateY(Math.PI);

    this.floorX = new THREE.Mesh(
      new THREE.PlaneBufferGeometry(this.size.x, this.size.y, 0),
      tjs_materials.flat_blue
    );
    this.floorX.name = "FloorPlaneX";
    this.floorX.translateX(this.size.horizCenter + 0.5);
    this.floorX.rotateY((3 * Math.PI) / 2);

    this.floorXP = new THREE.Mesh(
      new THREE.PlaneBufferGeometry(this.size.x, this.size.y, 0),
      tjs_materials.flat_blue
    );
    this.floorXP.name = "FloorPlaneXP";
    this.floorXP.translateX(-this.size.horizCenter + -0.5);
    this.floorXP.rotateY(HALF_PI);

    this.floorSpotZX = new THREE.Points(
      new THREE.BoxBufferGeometry(1, this.size.y, 0),
      tjs_materials.pointsY
    );
    this.floorSpotZX.name = "FloorSpotZX";

    this.floorSpotYX = new THREE.Points(
      new THREE.BoxBufferGeometry(this.size.z, 1, 0),
      tjs_materials.pointsX
    );
    this.floorSpotYX.rotateY(-HALF_PI);
    this.floorSpotYX.name = "FloorSpotYX";

    this.floorSpotYZ = new THREE.Points(
      new THREE.BoxBufferGeometry(this.size.x, 1, 0),
      tjs_materials.pointsZ
    );
  ...
  buildFloorIndicators() {
    for (let i = 0; i < this.size.y; i++) {
      var indicatorShape = new THREE.EdgesGeometry(
        new THREE.BoxBufferGeometry(this.size.x + 0.2, 1, this.size.z + 0.2)
      );
      var indicatorXform = new THREE.LineSegments(
        indicatorShape,
        tjs_materials.indicator_inactive
      );
  ...
  buildGrid() {
    loader.load("./models/dot.fbx", function (object) {
      object.traverse((child) => {
        if (child.isMesh) child.material = mat_flat_blue;
      });
  ...
  buildFoods() {
    ...
    for (let i = 0; i < this.numFood; i++) {
      let posCandidate = generateRandomPosition(
        ...
        (p, i) => p - (i === 1 ? this.size.vertCenter : this.size.horizCenter)
      );
      while (
        foodPositions.includes(posCandidate) ||
        arrayCompare([0, 0, 0], posCandidate)
      ) {
  ...
  checkCollision() {
    ...
    for (
      var vertexIndex = 0;
      vertexIndex < player.mesh.geometry.attributes.position.count;
      vertexIndex++
    ) {
      localVertex.fromBufferAttribute(
        player.mesh.geometry.attributes.position,
        vertexIndex
      );
      ...
      this.foods.map((food) => {
        if (food.eaten === undefined) {
          ...
          if (
            ...
            collisionResults[0].distance < directionVector.length() - 0.2
          ) {
            food.eaten = true;
    ...
    if (
      player.tail.trailRounded
        .slice(1, player.tail.trailRounded.length - 1)
        .some((trailPosition) => arrayCompare(trailPosition, player.position))
    )
      game.setGameOver();
  ...
  highlightFloor() {
    this.floorIndicators.map((indicator) => {
      if (Math.floor(indicator.position.y) === Math.floor(player.position[1]))
        indicator.material = tjs_materials.indicator;
  ...
  highlightFood() {
    this.foods.map((food) => {
      if (food.fbx && food.fbx.children[0]) {
        ...
        food.fbx.children[0].position.y +=
          Math.sin(CLOCK.elapsedTime * 2 + food.offset) / 400;

        if (player.position[1] === food.position.y) {
          if (
            player.position[0] === food.position.x ||
            player.position[2] === food.position.z
          ) {
  ...
  reset() {
    this.foods
      .concat(this.meshes)
      .concat(this.floorIndicators)
      .map((e) => tjs_scene.remove(e));
    this.foods = [];
    this.floorIndicators = [];
    player.reset();
  }
